    ...
    try {
      ...
      if (result) {
        ...
        if (result.statementId) {
          // We need the document's fileUrl to process it
          // The upload route returns the document which has the fileUrl
          statementIds.push({
            filePath: '', // will be filled by process-bulk from the BankStatement record
            fileName: file.name,
            fileSize: file.size,
